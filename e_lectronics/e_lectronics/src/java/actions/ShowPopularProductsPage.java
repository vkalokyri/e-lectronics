 ...
 * @author Valitsa
 */
public class ShowPopularProductsPage implements Action {
   ...
   public boolean execute(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
      ...
      Vector current=(Vector)session.getAttribute("CurrentMenu");
       ...
       try {
            popularProducts=prodDAO.getTheMostPopularProducts(new SimpleDateFormat("yyyy/MM/dd HH:mm:ss").format( new java.util.Date()),-1,orderBy);
            popularProducts.add(orderBy);
        } catch (SQLException ex) {
            Logger.getLogger(ShowFirstPage.class.getName()).log(Level.SEVERE, null, ex);
        }

       view = "./views/ShowPopularProductsView.jsp";
